...
class insert_prediction_data:
    """
    This class shall be used for inserting good prediction data into MongoDB Atlas database.
    ...
    def __init__(self, good_data_path, aws_operations=None, logger=None):
        ...
        self.csv_file_path = 'prediction_file_from_db/input_file_for_prediction.csv'
        self.log_file = 'prediction_logs/prediction_data_insertion_log.txt'
    ...
    def create_collection(self, collection_name, drop=True):
        ...
        On Failure:
            raise Exception
        """
        self.logger.log(self.log_file, 'Creating a collection for the prediction data!')
        try:
    ...
    def insert_good_data_into_collection(self, collection_name, index_column=None):
        """
        Method to insert the good prediction data into a collection.
        ...
        On Failure:
            raise Exception
        """
        self.logger.log(self.log_file, 'Inserting good prediction data into a collection!')
        try:
    ...
    def copy_data_from_collection_to_csv_file(self, collection_name):
        """
        Method to copy the prediction data from the collection to a .CSV file.
        ...
        On Failure:
            raise Exception
        """
        self.logger.log(self.log_file, 'Copying prediction data from the collection to a .CSV file!')
        try:
            conn = self.initialize_connection()
            collection_names = conn.list_collection_names()
