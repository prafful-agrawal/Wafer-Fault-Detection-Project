...
class validate_training_data:
    """
    This class shall be used for handling all the validation done on the training data.
    ...
    def __init__(self, folder_path, good_data_path, bad_data_path, aws_operations=None, logger=None):
        ...
        self.archive_folder_path = 'training_data_archive/'
        self.log_file = 'training_logs/training_data_validation_log.txt'
    ...
    def values_from_schema(self):
        """
        Method to extract all the relevant information from the pre-defined training schema.
        ...
        """
        self.logger.log(self.log_file, 'Extracting values from training schema!')
        try:
            schema = self.aws_operations.read_json(self.schema_path)
            if schema is None:
                raise Exception('Training schema file does not exists!')
            ...
            self.logger.log(self.log_file, 'Values from training schema extracted successfully!')
            return length_of_date_stamp_in_file, length_of_time_stamp_in_file, number_of_columns
        except ValueError:
            self.logger.log(self.log_file, 'ValueError: Value not found inside training schema!')
            raise ValueError
        except KeyError:
            self.logger.log(self.log_file, 'KeyError: Key value error, incorrect key passed!')
            raise KeyError
        except Exception as e:
            message = 'Error occurred while extracting values from the training schema: %s' % e
    ...
    def generate_file_name_regex(self, length_of_date_stamp_in_file, length_of_time_stamp_in_file):
        ...
        This regex pattern is used to validate the file name of the training data.
    ...
    def delete_existing_good_data_folder(self):
        """
        Method to delete the existing good training data folder.
        ...
        On Failure:
            raise Exception
        """
        self.logger.log(self.log_file, 'Deleting existing good training data folder!')
        try:
            if self.aws_operations.exists_folder(self.good_data_path):
                self.aws_operations.delete_folder(self.good_data_path)
            self.logger.log(self.log_file, 'Existing good training data folder deleted successfully!')
            return None
        except Exception as e:
            message = 'Error occurred while deleting existing good training data folder: %s' % e
            self.logger.log(self.log_file, message)
            raise e

    def delete_existing_bad_data_folder(self):
        """
        Method to delete the existing bad training data folder.
        ...
        On Failure:
            raise Exception
        """
        self.logger.log(self.log_file, 'Deleting existing bad training data folder!')
        try:
            if self.aws_operations.exists_folder(self.bad_data_path):
                self.aws_operations.delete_folder(self.bad_data_path)
            self.logger.log(self.log_file, 'Existing bad training data folder deleted successfully!')
            return None
        except Exception as e:
            message = 'Error occurred while deleting existing bad training data folder: %s' % e
            self.logger.log(self.log_file, message)
            raise e

    def archive_bad_training_data(self):
        """
        Method to move the bad training data to an archive.
        ...
        On Failure:
            raise Exception
        """
        self.logger.log(self.log_file, 'Moving bad training data to archive!')
        try:
            now = datetime.now()
            date = now.date()
            ...
            message = 'Error occurred while moving bad training data to archive: %s' % e
            self.logger.log(self.log_file, message)
            raise e

    def validate_file_names(self, regex):
        """
        Method to validate the file names as per the given training schema.
        ...
        On Failure:
            raise Exception
        """
        self.logger.log(self.log_file, 'Validating training file names!')
        try:
    ...
    def validate_number_of_columns(self, number_of_columns):
        """
        Method to validate the number of columns as per the given training schema.
        ...
        On Failure:
            raise Exception
        """
        self.logger.log(self.log_file, 'Validating number of columns in the training data!')
        try:
